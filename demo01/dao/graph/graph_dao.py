...
class GraphDao(object):
    ...
    def __init__(self):
        ...
        self.__database = Config.get_instance().get_with_nested_params("database", "neo4j", "database")
    ...
    def __connect_graph(self):
        try:
            self.__graph = Graph(self.__url, auth=(self.__username, self.__password), name=self.__database)
    ...
    @ensure_connection
    def query_task_steps_ordered_list(self, task_name):
        """
        查询指定任务的所有步骤，按执行顺序返回步骤名称列表
        """
        query = """
        MATCH path = (task:任务 {name: $task_name})-[:第一步]->(firstStep)-[:下一步*]->(lastStep)
        WHERE NOT (lastStep)-[:下一步]->()
        RETURN [node IN nodes(path) | node.name] as step_names
        """

        result = self.__graph.run(query, task_name=task_name).data()
        if result:
            # 返回完整的步骤名称列表，保持顺序
            return result[0]['step_names']
        else:
            return []
